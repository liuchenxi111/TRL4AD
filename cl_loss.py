...
def get_traj_cl_loss(gps_traj_rep, route_traj_rep, tau=0.07):    # 轨迹对比损失 (Trajectory Contrastive Loss)
    gps_traj_rep = F.normalize(gps_traj_rep, dim=1)              #形状为 (batch_size, feature_dim) 的张量，表示一批GPS轨迹的嵌入表示。
    route_traj_rep = F.normalize(route_traj_rep, dim=1)          #形状为 (batch_size, feature_dim) 的张量，表示一批路段轨迹的嵌入表示。
    logits = torch.mm(gps_traj_rep, route_traj_rep.t()) / tau    #计算GPS轨迹和路段轨迹之间的相似度（归一化的点积），并使用温度系数 tau 进行调整，控制平滑程度。
...
def get_road_cl_loss(gps_road_rep, route_road_rep, y_label, tau=0.07):  #路段对比损失 (Road Contrastive Loss)
...
def get_traj_match_loss(self,gps_traj_rep, route_traj_rep, model, batch_size=64, tau=0.07):
    ...
    label = torch.cat([torch.ones(batch_size, dtype=torch.long), torch.zeros(2 * batch_size, dtype=torch.long)],dim=0).to(self.device) # 1 x 64*3
